...
| A caller that had never parsed merged clean: a job-level `if:` referencing `matrix` made `evolve.yml` unreadable, so every push filed a `startup_failure` run and its Sunday cron never armed | The detection existed and worked — `agent status --failures` already counted `startup_failure` (#133) and showed the file path in place of a workflow name, the signature of an unparseable file, four rows deep — but nothing runs that command unprompted. The signal was present, correct, and unread for a day, until a separate CI lane filing #213 caught it by accident | `actionlint` in CI fails a PR before an unparseable workflow can merge; every scheduled groom run checks that repo's own currently-enabled workflows for `startup_failure` and opens a `needs-human` issue, so the signal arrives without anyone typing the command. A deliberately disabled workflow is excluded so an expected failure can't retrain a human to ignore this the way the missing arrival did. Still gated on the pipeline's own idle guard (no run fires on a repo with zero open issues), which this does not touch (#217) |
...
| The undeclared case (default `commands.requires: []`) still burned the full attempt budget: a gate whose binary was never declared, so the #246 preflight had nothing to check, hit `sh -c '<gate>'` exiting 127 with `sh: 1: <tool>: not found` mid-run. `run_gates` recorded only `returncode == 0`, so that read exactly like a real gate failure and the loop retried it `max_attempts` times, rewriting working code against a wall no code change could fix | `run_gates` classifies a 127 exit whose stderr says "not found" as `GateStatus.MISSING` — a gate that never ran — distinct from `GateStatus.FAILED`; a script that deliberately `exit 127`s with no such text still reads as a real failure, and a timeout keeps meaning "failure" too. `run_task_loop` aborts on the first `MISSING` result instead of retrying, and `implement`/`resume` report it as an environment gap (naming the gate and the binary) rather than a gate failure — keeping the worktree, since an attempt already ran and may hold a real diff (#287) |
