...
def test_make_plan_includes_issue_comments_in_rendered_prompt(
    ...
    assert "(nothing is configured; do not invent setup or gate commands)" in captured["prompt"]
    assert "verification context for this read-only lane" not in captured["prompt"]
    assert "Do not run the configured `setup` command here" not in captured["prompt"]
    assert (
...
def test_run_implement_with_grant_records_authorization_and_scope_in_the_prompt(
    ...
    assert "jirathip-k" in captured["prompt"]
    assert "(nothing is configured; do not invent setup or gate commands)" in captured["prompt"]
    assert "Run the configured gates yourself before finishing" not in captured["prompt"]
    assert "parent `run_gates`" not in captured["prompt"]
