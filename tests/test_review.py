...
def test_run_review_under_budget_includes_full_diff_no_note(
    ...
    assert (
        "(nothing is configured; do not invent setup or gate commands)" in runtime.received_prompt
    )
    assert "verification context for this read-only lane" not in runtime.received_prompt
    assert "UNVERIFIED" not in runtime.received_prompt
    assert "parent `run_gates`" not in runtime.received_prompt
